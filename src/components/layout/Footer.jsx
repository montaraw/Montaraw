...
const socialLinks = [
  {
    name: 'Instagram',
    key: 'instagram',
    url: 'https://www.instagram.com/montaraw_clothinghouse?igsi=MjJ2NWdrMGRtYzM1',
    icon: (
  ...
  },
  {
    name: 'Facebook',
    key: 'facebook',
    url: 'https://www.facebook.com/share/17Vh8emhBD/',
    icon: (
      <svg className="w-4 h-4 fill-current" viewBox="0 0 24 24">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 md:gap-10">
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center gap-2.5 pt-1">
              {socialLinks.map((social) => {
                const targetUrl = (social.key === 'instagram' && settings.instagram)
                  ? settings.instagram
                  : ((social.key === 'facebook' && settings.facebook) ? settings.facebook : social.url);
                ...
                return (
                  <a
                    ...
                    className="w-10 h-10 rounded-xl flex items-center justify-center border border-white/20 bg-white/10 text-white hover:bg-brand-red hover:border-brand-red transition-all shadow-md"
                    title={`Follow Montaraw on ${social.name}`}
                  >
                    {social.icon}
                  </a>
                );
          ...
          <div className="hidden md:block">
            ...
            <ul className="space-y-2.5 text-xs text-gray-200">
              ...
              <li>
                <Link to="/shop/pakistani-suits" className="hover:text-white transition-colors text-white font-semibold pl-2 border-l border-white/20">
                  Pakistani Suits
                </Link>
              </li>
              <li>
                <Link to="/shop/suits" className="hover:text-white transition-colors text-white font-semibold pl-2 border-l border-white/20">
                  Suits & Anarkalis
                </Link>
              </li>
              <li>
                <Link to="/shop/cord-set" className="hover:text-white transition-colors text-white font-semibold pl-2 border-l border-white/20">
                  Cord Set Collections
                </Link>
              </li>
              <li>
              ...
              <li>
                <Link to="/shop?new=true" className="hover:text-white transition-colors">
                  New Arrivals '25
                </Link>
              </li>
              <li>
          ...
          <div>
            ...
            <ul className="space-y-2.5 text-xs text-gray-200">
              ...
              <li>
                <span className="text-gray-400 block text-[10px] uppercase font-bold">Primary Helpline:</span>
                <a
                  href={`tel:${(settings.contactPhone || '+919720538576').replace(/[^+\d]/g, '')}`}
                  className="text-white hover:text-brand-red font-medium transition-colors inline-block"
                >
                  {settings.contactPhone || '+91 97205 38576'}
                </a>
              </li>
              <li>
                <span className="text-gray-400 block text-[10px] uppercase font-bold">Secondary Helpline:</span>
                <a
                  href={`tel:${(settings.contactPhoneSecondary || '+916206424372').replace(/[^+\d]/g, '')}`}
                  className="text-gray-300 hover:text-white font-medium transition-colors inline-block"
                >
                  {settings.contactPhoneSecondary || '+91 62064 24372'}
                </a>
              </li>
              <li>
                <span className="text-gray-400 block text-[10px] uppercase font-bold">Official Social Channels:</span>
                <div className="flex items-center gap-2.5 pt-1 text-xs">
                  <a
                    href={settings.instagram || 'https://www.instagram.com/montarawsupport?igsi=MjJ2NWdrMGRtYzM1'}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="text-gray-200 hover:text-brand-red font-medium transition-colors"
                  >
                    Instagram
                  </a>
                  <span className="text-gray-500">•</span>
                  <a
                    href={settings.facebook || 'https://www.facebook.com/share/17Vh8emhBD/'}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="text-gray-200 hover:text-brand-red font-medium transition-colors"
                  >
